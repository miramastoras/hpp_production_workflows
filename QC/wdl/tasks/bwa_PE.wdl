...
# run BWA for PE reads. Requires exactly two fastq files as input
...
task BwaAlignment{
    ...
    command <<<
        ...
        # bwa alignment
        bwa mem -a -M ~{bwaParams} -t~{threadCount} asm.fa ~{sep=" " readFastq} | samtools view -b -h > ~{outputName}.bam
